                    ...
                    <table className="min-w-full">
                        <thead>
                            <tr className="bg-blue-600 text-white">
                                ...
                                <th className="px-6 py-4 text-left text-sm font-medium">মেয়াদকাল</th>
                                <th className="px-6 py-4 text-left text-sm font-medium">যোগাযোগ</th>
                                <th className="px-6 py-4 text-left text-sm font-medium">ইমেইল</th>
                                <th className="px-6 py-4 text-left text-sm font-medium">ঠিকানা</th>
                            </tr>
                        </thead>
...
<path d="M10 54c0-12.15 9.85-22 22-22s22 9.85 22 22" fill="#9CA3AF"/>
                                    ...
                                    <td className="px-6 py-4 text-sm text-gray-900">{person.phone}</td>
                                    <td className="px-6 py-4 text-sm text-blue-600">{person.email}</td>
            ...
            <div className="lg:hidden space-y-4">
                {data.map((person, index) => (
                    <div key={person.id} className="bg-white rounded-lg shadow-md p-6 border-l-4 border-blue-600">
                            ...
                            <div className="flex-1 text-center sm:text-left">
                                ...
                                <div className="space-y-2">
                                    <div className="flex flex-col sm:flex-row sm:items-center space-y-1 sm:space-y-0">
                                        <span className="text-sm font-medium text-gray-600 w-full sm:w-20">মেয়াদকাল:</span>
                                    ...
                                    <div className="flex flex-col sm:flex-row space-y-1 sm:space-y-0">
                                        <span className="text-sm font-medium text-gray-600 w-full sm:w-20">যোগাযোগ:</span>
                                        <span className="text-sm text-gray-900">{person.phone}</span>
                                    </div>
                                    <div className="flex flex-col sm:flex-row space-y-1 sm:space-y-0">
                                        <span className="text-sm font-medium text-gray-600 w-full sm:w-20">ইমেইল:</span>
                                        <span className="text-sm text-blue-600 break-all">{person.email}</span>
                                    </div>
                                    <div className="flex flex-col sm:flex-row space-y-1 sm:space-y-0">
                                        <span className="text-sm font-medium text-gray-600 w-full sm:w-20">ঠিকানা:</span>
                                        <span className="text-sm text-gray-900">{person.address}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        <div className="min-h-screen bg-gray-100">
            <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8">
                ...
                <div className="text-center mb-8">
                    <h1 className="text-3xl sm:text-4xl font-bold text-gray-800 mb-2">প্রশাসন</h1>
                ...
                <div className="min-h-[400px]">
                    {activeTab === 'সভাপতি সারণী' && renderDataCards(presidentData, 'সভাপতি সারণী')}
                    {activeTab === 'সাধারণ সম্পাদক সারণী' && renderDataCards(secretaryData, 'সাধারণ সম্পাদক সারণী')}
                </div>
            </div>
        </div>
    );
